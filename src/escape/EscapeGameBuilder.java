...
import escape.board.pathfind.VersionProvider;
...
public class EscapeGameBuilder
{
	 ...
	 * 
	 * @param fileName
	 *            the file for the Escape game configuration file (.egc).
	 * @throws Exception
	 *             on any errors
	 */
	...
	/**
	 * Take the .egc file contents and turn it into XML. If you want to use JSON, you
	 * should change the name of this method and initizlize the EscapeConfigurator with
	 * this code: EscapeConfigurator configurator = new
	 * EscapeConfigurator(EscapeConfigurationJsonMaker());
	 * 
	 * @param fileName
	 ...
	 */
	private EscapeGameInitializer unmarshalXml(String xmlConfiguration)
			throws JAXBException
	{
		JAXBContext contextObj = JAXBContext
				.newInstance(EscapeGameInitializer.class);
		Unmarshaller mub = contextObj.createUnmarshaller();
		return (EscapeGameInitializer) mub
				.unmarshal(new StreamSource(new StringReader(xmlConfiguration)));
	}

	/**
	 * Getter for the gameInitializer. Can be used to examine it after the builder creates
	 * it.
	 * 
	 ...
	 * instance. For this example, you would use the gameInitializer object to get all of
	 * the information you need to create your game.
	 * 
	 * @return the game instance
	 */
	public EscapeGameManager makeGameManager()
	{
		VersionProvider.setCoordinateType(gameInitializer.getCoordinateType());
		EscapeGameManager egm = new EscapeGameManagerImpl(gameInitializer);
		return egm;
	}
}
